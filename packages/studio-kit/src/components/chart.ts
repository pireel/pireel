...
#${id} .title{font-size:${s.label}px;font-weight:700;${shadow}}
...
#${id} .center .cl{font-size:${s.kicker}px;color:${inkOn(p, true)};${shadow}}
</style>`;
...
#${id} .cv{font-size:${s.kicker}px;${shadow}}
#${id} .cl{font-size:${s.kicker}px;color:${inkOn(p, true)};margin-top:${Math.round(s.gap * 0.2)}px;${shadow}}
</style>`;
    if (title) tl.push(fadeUp(`#${id} .title`, 0.05, { dur: 0.3 }));
...
#${id} .cl{flex:0 0 ${Math.round((ctx.box.w - s.pad * 2) * 0.22)}px;font-size:${s.kicker}px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;${shadow}}
...
#${id} .cv{flex:0 0 auto;font-size:${s.kicker}px;${shadow}}
</style>`;
    if (title) tl.push(fadeUp(`#${id} .title`, 0.05, { dur: 0.3 }));

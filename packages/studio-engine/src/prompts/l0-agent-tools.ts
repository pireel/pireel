...
export const STUDIO_TOOLS: StudioToolDef[] = [
  ...
  /* ---------- export (local client-side compositing, card · slow) ---------- */
  {
    ...
    description:
      "Export the final video. TWO STEPS, and it will NOT export until step 2. STEP 1 — call it with NO confirmed: it returns source- and platform-tuned recommendations (Xiaohongshu / Douyin·TikTok / YouTube / source-quality) in `data.options`. Put those to the user with ask_user (one option per recommendation) and let THEM pick — never choose the resolution/fps/format yourself, and never set confirmed:true on your own. STEP 2 — after the user picks, call again with their chosen `resolution`/`fps`/`format` AND `confirmed:true` to actually start (only if the user's original message already named specs may you go straight to step 2). It renders LOCALLY in the user's open studio tab (roughly realtime: a 3-min video takes ~3 min) and saves via the browser's download — nothing is uploaded. Poll track_export for progress and the final filename; the tab must stay open until done. Driving a headless/embedded browser yourself? Those often DISCARD downloads — run the export-sink helper first and pass its `sink_url` so the file is delivered to disk reliably.",
    inputSchema: obj(
      {

...
export const STUDIO_TOOLS: StudioToolDef[] = [
  ...
  /* ---------- export (local client-side compositing, card · slow) ---------- */
  {
    ...
    description:
      "Export the final video. When the user asks to export, CALL THIS TOOL in that same turn — announcing it in text does NOTHING: the settings card only exists as this tool call's card in the stream, so a reply without the call leaves the user staring at nothing (this failure has happened; do not repeat it). Call with NO arguments; an export-settings card renders INLINE right at the call (not a popup — never call it one) with resolution/fps/format controls, and the call completes once the user hits Export. A short line in the user's language may accompany the call, but the call itself is mandatory; never list the options as prose and never choose the specs yourself. (Only when the user's message already named the specs, call with those `resolution`/`fps`/`format` AND `confirmed:true` to skip the card.) It renders LOCALLY in the user's open studio tab (roughly realtime: a 3-min video takes ~3 min) and saves via the browser's download — nothing is uploaded. Poll track_export for progress and the final filename; the tab must stay open until done. Driving a headless/embedded browser yourself? Those often DISCARD downloads — run the export-sink helper first and pass its `sink_url` so the file is delivered to disk reliably.",
    inputSchema: obj(
      {

...
- Tightening pauses IS a legitimate pass: collect inter-sentence gaps from the transcript rows (gap = next row's start − this row's end) and remove them with ONE cut_narration call, passing the FULL gap ranges plus keepGapSec — the tool leaves that much breathing room at each seam. NEVER do the margin arithmetic yourself; your job is choosing WHICH gaps, the tool's job is the boundary math. When you report the result, quote the receipt's data.cuts / removedTotalSec (seconds ACTUALLY removed after margins) — your raw gap sums overstate the cut and won't match what the user sees in the editor.

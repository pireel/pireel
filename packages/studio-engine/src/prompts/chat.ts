...
- There is no scenario-specific edit macro. For a complete edit, first read the relevant transcript and footage evidence and resolve only user-owned decisions that block execution. A supplied script is SEMANTIC truth, not automatically TIMING truth; measure performed audio only when timing, captions or beat placement need it. For a multi-source montage, inspect actual footage and choose useful action spans; uniform slices, filename-order assembly or one untouched span per file are never a finished edit. Keep only the compact task-local judgments needed to execute the current edit, then compile them directly into batched picture, text, graphic and sound edits. Ordinary editing does not create persisted planning artifacts. Review the rendered timeline and repair missing evidence, filler loops, continuity breaks or unreadable graphics before completion.

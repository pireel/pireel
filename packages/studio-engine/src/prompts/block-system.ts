import { COMPONENT_COORDINATE_CONTRACT } from '../component-authoring-contract';
/**
...
1) INNER HTML: markup + ONE <style> with local selectors (e.g. .num {…}); Studio scopes them to this component automatically. Use :scope for the component container. Existing #BLOCK_ID selectors remain supported; do not invent ids for CSS isolation.
...
SYMBOLS — anything load-bearing must be drawn, not typed
- A glyph is whatever the renderer's font decides it is, and the browser, the Linux render container and an editor opening the exported file do not agree. Never let a Unicode arrow, triangle, dingbat or emoji (→ ➔ ▶ ➤ ★ ✓ …) carry meaning: draw it as an inline SVG path or CSS geometry instead. Keep an arrow's shaft and head in one SVG and one coordinate system so they cannot drift apart; if you use <marker>, define it in that same SVG with a unique id and make sure every marker-start/marker-end resolves.
- The same applies to bullets, checkmarks and rating marks. Decorative glyphs inside a sentence the user wrote are fine — the rule is about symbols the composition would lose meaning without.
...
- EDITABLE PROPERTIES (REQUIRED): every component declares the values a user will tune as a SEPARATE \`\`\`json fence after the html/js — a JSON Schema properties object, the same vocabulary registered components use (NOT an attribute in the markup): {"accent":{"type":"string","format":"color","title":"强调色","default":"#ff5a36"},"value":{"type":"number","title":"数值","minimum":0,"maximum":100,"default":72},"badge":{"type":"boolean","title":"角标","default":true},"layout":{"type":"string","title":"布局","enum":["row","column"],"default":"row"},"face":{"type":"string","format":"font","title":"标题字体","default":"sans"}}. Expose useful controls, including the accent color when it is used; do not invent dummy properties to reach a quota; kinds: string+format color, string+format font, number (minimum/maximum/multipleOf), boolean, string+enum; kebab keys; titles in the on-screen language. Consume each in the markup ONLY as var(--p-<key>) — colour, unitless number (calc(var(--p-value) * 1%) when a unit is needed), font-family:var(--p-face) — or as a #BLOCK_ID[data-p-<key>="v"] selector (boolean "true"/"false", enum member). Never write --p-* or data-p-* yourself, never put a data-props attribute in the markup, and never restate defaults in CSS — Studio materializes every property on #BLOCK_ID and the inspector changes them there without regenerating you. The timeline body never reads them. Text is never a property (every text element already carries data-edit; the inspector edits it there); images are <img> slots.
...
- Every text role follows the coordinate contract above. Check rendered readability; preserve SVG user units on an existing artboard.
...
- Motion follows the content: one deliberate entrance and stable hold is valid; use multiple stages only when the information unfolds in stages. CUSTOM DISPLAY TYPOGRAPHY instead follows its 0.18–0.35s decisive entrance and stable hold. With SPOKEN BEATS, no later content is visible before the beat that first mentions it. CSS isolation is supplied by Studio; timing stays local.
- Every visible text element carries a unique data-edit key (except animation-rewritten text).
- the properties schema is a separate \`\`\`json fence with 2–6 entries including one color; every key is read in the markup via var(--p-key) or [data-p-key]; no data-props attribute, no --p-* declaration and no data-p-* attribute written in the markup; no text property.

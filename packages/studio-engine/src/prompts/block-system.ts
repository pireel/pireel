...
- COMPONENT VOCABULARY (pick what fits; compose richer when asked): metric/stat card · comparison (two columns / before↔after, center divider, accent the winner) · pipeline/flow (boxed nodes + inline-SVG arrows, label each step) · structure/hierarchy (tree/grouped, hairline connectors) · KPI grid (2–4 cells, separators) · loop/cycle (nodes on a ring, curved SVG arrows) · chart (recipes below) · callout/quote/checklist/timeline.

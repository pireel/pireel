...
- An empty timeline is not a blocker: place the library footage with add_clips (role primary), then edit it. A new cut over an output that already holds a finished picture goes into a new output (manage_project) unless the user asked to replace it; the old cut stays one switch away.

...
- manage_project chooses and pins the project you edit. The connected tab and server tools share this project anchor; another project's autosave does not change it. Newest-touched is used only when no session project has been selected. After explicitly creating or selecting a project, server tools and cloud imports target it even if an older tab remains open; data edits use the selected cloud project and browser-only tools require opening it. get_state preserves an explicit selection. Use manage_project to select a different project. An unexpected later tab takeover does not retarget server tools.

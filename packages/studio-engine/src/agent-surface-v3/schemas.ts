...
export const V3_TOOL_SCHEMAS: Record<string, V3ToolSchema> = {
  ...
  assemble_from_review: {
    description:
      'Build the primary picture track from your ordered picks (montage work; B-roll over a talking head is add_clips). clips[] come from inspect_media mode:editorial — source [inSec,outSec] inside accepted ranges — and are placed as written in that order, snapped to legal action boundaries at most, at natural speed; the current primary picture is replaced (one undo step). Nothing is chosen for you: what your picks do not cover stays open. Target = targetDurationFrames, else the narration on the timeline. Receipt: coverage {targetDurationSec, actualDurationSec, shortfallSec, covered}, placed[] (each clip with its review evidence), and when short, remaining[] — the unused accepted ranges with score, action and facing — so you add picks and call again with the complete ordered list, or tell the user the pool is short. Omit clips to start from the review\'s opening choice.',
    inputSchema: obj({

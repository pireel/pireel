...
export const V3_TOOL_SCHEMAS: Record<string, V3ToolSchema> = {
  ...
  inspect_media: {
    description:
      `Look at media and components by mode. metadata (default): registered assets or clips — dimensions, duration, transcript coverage, placements, generationStatus for pending generations. frames: the pixels of 1–8 image assets. component: one graphic clip's markup and animation. generation: status of pending image/video/audio jobs (omit ids for recent history). geometry: free and browser-local — scene cuts, subject tracks and safe regions. semantic / editorial: ${CHARGE_MARKER} content understanding or a comparative review of ranges against a brief. brief then labels: BYO visual analysis — the tab returns sample frames, you label them, submit with labels. For the composed timeline use inspect_timeline instead.`,
    inputSchema: obj({

...
icon: ⚪
showcase: [title-card, quote, qa, big-number, timeline, list, cta]
...
- list: a sparse vertical index — ONE vertical hairline standing at the 38.2% column; three serif items (64px, weight 500, +0.12em) hang to its right with 90px+ gaps, each led by a tiny 28px muted numeral; the key item ALONE carries the cinnabar dot at its end; a 30px Latin caption waits far away in the lower-left. Motion: hairline scales from top 0.7s, items drift up 10px in slow sequence, dot fades last.
- cta: a near-empty page — the one hairline at `top:61.8%`; a small serif `关注` (96px, +0.2em) resting just above it at `left:150px` with the cinnabar dot beside it; a vertical-rl side note hugging the right edge; one 30px Latin caption below the line. Motion: slow fades ONLY (0.6-0.8s), no drift, dot last.

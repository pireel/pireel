...
export interface ReviewedPlacementIssue {
  assetId: string;
...
/** Once a source has been reviewed, a plain add_clips/insert_clips placement of it must stay inside
 * an accepted range: the persisted verdict is authoritative over a later step's memory of it. */
export function reviewedPlacementIssue(
  sources: readonly EditorialAssemblySource[],
  clips: readonly unknown[],
): ReviewedPlacementIssue | null {
  if (!sources.length) return null;
  const reviewed = new Map(sources.map((source) => [source.assetId, source.candidates]));
  for (const value of clips) {
    if (!value || typeof value !== 'object' || Array.isArray(value)) continue;
    const clip = value as Record<string, unknown>;
    const assetId = canonicalReviewedAssetId(clip.assetId);
    const candidates = reviewed.get(assetId);
    if (!candidates) continue;
    const ranges = accepted(candidates)
    ...
    if (!ranges.length) return { assetId, reason: 'review-rejected' };
    const acceptedRanges = ranges.slice(0, 8);
    ...
    const inside = ranges.some((range) => sourceInSec >= range.startSec - tolerance && sourceOutSec <= range.endSec + tolerance);
    if (!inside) return { assetId, reason: 'outside-accepted-range', requestedRange: { sourceInSec, sourceOutSec }, acceptedRanges };
  }
  return null;
}

export function describeReviewedPlacementIssue(issue: ReviewedPlacementIssue): string {
  const sec = (value: number) => `${Math.round(value * 100) / 100}s`;
  const acceptedList = issue.acceptedRanges?.length
    ? ` Accepted: ${issue.acceptedRanges.map((range) => `${sec(range.startSec)}–${sec(range.endSec)}`).join(', ')}.`
    : '';
  const head = issue.reason === 'review-rejected'
    ? `Source ${issue.assetId} has no accepted editorial range.`
    : issue.reason === 'range-required'
      ? `Source ${issue.assetId} is reviewed; place it with an explicit accepted source range.${acceptedList}`
      : `Source ${issue.assetId} range ${sec(issue.requestedRange!.sourceInSec)}–${sec(issue.requestedRange!.sourceOutSec)} falls outside its accepted editorial ranges.${acceptedList}`;
  return `${head} Nothing was placed — do not undo; retry inside an accepted range, or call assemble_from_review to build the picture from the reviewed pool.`;
}

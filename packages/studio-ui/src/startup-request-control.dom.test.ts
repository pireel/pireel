...
vi.mock("./chat-thread-store", () => ({
  ...
  createStudioTurnLedger: () => ({
    receipts: [], toolCallCount: 0, forceFinalResponse: false, pictureLocked: false,
    postAssemblyTimelineRead: false, blockFurtherTimelineReads: false,
    unsafeUndoBlocked: false, lastTimelineFingerprint: null,
  }),
  editorialPlacementIssue: () => null,
  effectiveStudioTurnMessages: (messages: unknown) => messages,
  hasCompletedEditorialPlacement: () => false,
  hasPostAssemblyTimelineSnapshot: () => false,
  isRecoverableStudioChatError: () => false,
  narrationDurationFromMessages: () => 0,
  prepareEditorialPlacement: () => null,
  recordStudioTurnToolResult: () => ({ timelineUnchanged: false }),
